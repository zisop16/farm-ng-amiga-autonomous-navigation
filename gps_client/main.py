...
def print_ecef_frame(msg):
    ...
    print(f"Coordinates: ({(msg.x, msg.y, msg.z)})")
...
async def main(service_config_path: Path) -> None:
    ...
    async for event, msg in EventClient(config).subscribe(config.subscriptions[0]):
        if isinstance(msg, gps_pb2.EcefCoordinates):
            time = msg.gps_time.stamp
            if initial_time is None:
                initial_time = time
            pos = (msg.x, msg.y, msg.z)
            coordinates.append(str(pos))
            if time > initial_time + logging_duration:
                break
    with open("coordinates.txt", 'w') as coordinates_file:
