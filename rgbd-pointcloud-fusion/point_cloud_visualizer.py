...
class PointCloudVisualizer:
    def __init__(self, cameras: List[Camera]):
        ...
        self.point_cloud_window.register_key_callback(
            ord("S"), lambda vis: self.save_point_cloud()
        )
        self.point_cloud_window.register_key_callback(
    ...
    def save_point_cloud(self):
        for camera in self.cameras:
            o3d.io.write_point_cloud(
                f"sample_data/pcl_{camera.mxid}.ply", camera.point_cloud
            )
        combined = self.cameras[0].point_cloud + self.cameras[1].point_cloud
        o3d.io.write_point_cloud(f"sample_data/combined.ply", combined)
